...
#Reading a day1.csv file
df = spark.read.format("csv").option("header", "true").option("inferschema", "true").load("D:\BigData\Pyspark\PysparkProjects\CurrencyConversions\Datasets/day1.csv")
df.show()
...
#Adding incremental subjectid and batchid
finaldf = addColumnIndex(spark,procdf).withColumn("batchid", lit(1))
finaldf.show()
...
finstrdf.write.format("jdbc").option("url", "jdbc:mysql://localhost:3306/test").option("driver", "com.mysql.cj.jdbc.Driver").option("user", "root").option("password", "123456").option("dbtable", "pycurrencytab").save()